...
void WhirlpoolClimateAC::transmit_state() {
  ...
  if (this->ir_sensor_ != nullptr && this->ir_sensor_->state == "On") {
     transmit.perform();
  }
}
...
bool WhirlpoolClimateAC::on_receive(remote_base::RemoteReceiveData data) {
  ...
  auto t_diff = ::difftime(t_receive, t_transmit);
  //ESP_LOGD(TAG, "t_transmit: %s , t_receive: %s , diff: %f", ctime(&t_transmit), ctime(&t_receive), t_diff);
  ESP_LOGD(TAG, "t_transmit: %u , t_receive: %u , diff: %f", t_transmit, t_receive, t_diff);
  if (::abs(t_diff) < 1) {
    ESP_LOGD(TAG, "t_transmit: %u , t_receive: %u , diff: %.f", t_transmit, t_receive, t_diff);
    return false;
  }
